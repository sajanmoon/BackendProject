...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "something went wrong while genarating refresh access token"
    );
  }
};
...
const updateAccountSuccessfully = asyncHandler(async (req, res) => {
  const { fullName, email } = req.body;

  if (!fullName || !email) {
    throw new ApiError(400, "All feild are reqired");
  }
